...
def check_token(conn, token):
    return conn.hget('login:', token)   #A  hash操作, 返回 token 对应的 user
...
def update_token(conn, token, user, item=None):
    ...
    conn.hset('login:', token, user)                    #B   hash操作, 在 login: 里记录 token -> user 
    conn.zadd('recent:', token, timestamp)              #C   有序集合, 在 recent: 里按 timestamp 对 token 排序
    if item:
        conn.zadd('viewed:' + token, item, timestamp)   #D   有序集合, 在 viewed:$token 里按 timestamp 对 item 排序
        conn.zremrangebyrank('viewed:' + token, 0, -26) #E   保存最后 25 个记录, 即最近访问的 25 个记录
...
.We just logged-in/updated token: 54349842-af84-42ad-89ee-f56197e18365
For user: username
...
Our shopping cart currently has: {'itemY': '3'}
