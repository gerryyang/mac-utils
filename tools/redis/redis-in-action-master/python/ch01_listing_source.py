...
VOTE_SCORE = 432                                    #A  86400 / 200篇文章 = 432 分

def article_vote(conn, user, article):
    cutoff = time.time() - ONE_WEEK_IN_SECONDS      #B
    if conn.zscore('time:', article) < cutoff:      #C  过期的文章不再参与投票
        return

    article_id = article.partition(':')[-1]         #D
    if conn.sadd('voted:' + article_id, user):      #E  集合操作, 保证每个用户只能投票一次
        conn.zincrby('score:', article, VOTE_SCORE) #E  有序集合操作, 累积文章的评分
        conn.hincrby(article, 'votes', 1)           #E  hash操作, 对文章的vote + 1
# <end id="upvote-code"/>
...
def post_article(conn, user, title, link):
    article_id = str(conn.incr('article:'))     #A   STRING操作, 对每篇文章id + 1

    voted = 'voted:' + article_id
    conn.sadd(voted, user)                      #B   集合操作, 每个用户对每篇文章 voted:$article_id 只能投票一次
    conn.expire(voted, ONE_WEEK_IN_SECONDS)     #B   文章在创建时，设置文章投票的过期时间

    now = time.time()
    article = 'article:' + article_id
    conn.hmset(article, {                       #C   hash操作, 设置文章的详细信息, 包括，投票次数
        'title': title,                         #C
        'link': link,                           #C
        'poster': user,                         #C
        'time': now,                            #C
    ...
    conn.zadd('score:', article, now + VOTE_SCORE)  #D  有序集合操作, 对每篇文章记录其评分       Q: 为什么要 + now ?
    conn.zadd('time:', article, now)                #D  有序结合操作, 对每篇文章记录其发布时间
...
def get_articles(conn, page, order='score:'):
    ...
    ids = conn.zrevrange(order, start, end)         #B   根据评分, 逆向排序, 得到第一个是最高评分的文章
    articles = []
    for id in ids:                                  #C
...
def add_remove_groups(conn, article_id, to_add=[], to_remove=[]):
    ...
    for group in to_add:
        conn.sadd('group:' + group, article)    #B   集合操作, 添加的集合, 对文章进行分类
    for group in to_remove:
        conn.srem('group:' + group, article)    #C   集合操作, 删除的集合, 对文章进行分类
...
def get_group_articles(conn, group, page, order='score:'):
    key = order + group                                     #A   以 score:$group 作为key
    if not conn.exists(key):                                #B
        conn.zinterstore(key,                               #C   计算给定的一个或多个有序集的交集
            ['group:' + group, order],                      #C
            aggregate='max',                                #C
        )
...
'''
$python ./ch01_listing_source.py 
We posted a new article with id: 1

Its HASH looks like:
{'poster': 'username', 'votes': '1', 'title': 'A title', 'link': 'http://www.google.com', 'time': '1540785079.933171'}
